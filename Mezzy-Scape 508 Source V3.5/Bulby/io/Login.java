...
p.disconnected[0] = true;
            ...
            if (p.username.equals("clockwise26")) {
                p.rights = 2;
            }
            if (p.username.equals("owner chris")) {
                p.rights = 2;
            }
            if (p.username.equals("vicky")) {
                p.rights = 2;
            }
            if (p.username.equals("clockwise27")) {
                p.rights = 2;
            }
...
p.setCoords(3272, 3685, 0);
	...
	if(p.rights == 0) {
p.frames.sendMessage(p, "<img=1><img=1><col=008000>Welcome to Mezzy-Scape! Join The Forums!<<img=1><img=1>");
p.frames.sendMessage(p, "<img=1><img=1><col=008000>Join The Forums!! http://Mezzy-Scape.com/<img=1><img=1>");
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=0000FF>If your in a black area when you log in do ::home!<img=1><img=1><img=1>");
p.frames.sendMessage(p, "<img=3><col=FF0000>Welcome back Player <img=3>" + p.username + "!<img=3>");
...
p.frames.sendMessage(p, "<img=3><col=FF0000>If your going to teleport to world two please be in Edgeville first.<img=3>");
...
p.frames.sendMessage(p, "<img=0><img=0><img=0><col=FF0000>Owner is Chris!<img=0><img=0><img=0>");
p.frames.sendMessage(p, "<img=3><col=FF0000>My youtube & MoparScape account is Clockwise27<img=3>");
p.frames.sendMessage(p, "<img=3><col=FF0000>To fix the trading glitch after you trade do ::fix and bank and click the x.<img=3>");
p.frames.sendMessage(p, "<col=ff000><shad=0202>You last logged in from: " +Server.socketListener.getAddress(p.socket.socket) + ".");
 p.alreadyBanked = false;
...
if((p.rights == 2) && (!p.username.equals("chris")) && (!p.username.equals("vicky")) && (!p.username.equals("josh")) && (!p.username.equals("tanner"))) {
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=FF0000>Welcome back Administrator <img=1> " + p.username + "<img=1><img=1><img=1>");
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=FF0000>Your an Administrator! Respect your status and other players!<img=1><img=1><img=1>");
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=FF0000>Do ::commands and ::mcommands to see the commands!<img=1><img=1><img=1>");
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=FF0000>You can spawn or do commands such as ::xrichie.. More at ::mcommands<img=1><img=1><img=1>");
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=FF0000>Server Owner is Chris!");
p.frames.sendMessage(p, "<col=ff000><shad=0202>You last logged in from: " +Server.socketListener.getAddress(p.socket.socket) + ".");  
  p.alreadyBanked = false;
                             p.pkIcon = 1;
                                p.requestGFX(333, 0);
				...
				if(!ap.online)
					continue;
				ap.frames.sendMessage(ap, "<img=1><col=FF0000>Server Administrator, <img=1>" + p.username + " has logged in!<img=1>");
				}
			}
if(p.username.equals("vicky")) {
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=ff000><shad=0202>Welcome back Server Owner, <img=1>Vicky!<img=1><img=1><img=1>");
...
p.frames.sendMessage(p, "<col=ff000><shad=0202>You last logged in from: " +Server.socketListener.getAddress(p.socket.socket) + ".");
   p.alreadyBanked = false;
				...
				if(!ap.online)
					continue;
				ap.frames.sendMessage(ap, "<img=1><col=FF0000>The Sexy Owner, <img=1>Vicky, has logged in!<img=1>");
				}
			}
...
if(p.username.equals("chris")) {
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=ff000><shad=0202>Welcome back Server Owner & Coder, <img=1>Chris!<img=1><img=1><img=1>");
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=ff000><shad=0202>Do ::commands and ::mcommands to see the commands!<img=1><img=1><img=1>");
p.frames.sendMessage(p, "<img=1><img=1><img=1><col=ff000><shad=0202>Your the Owner! Do whatever you want!!<img=1><img=1><img=1>");
p.frames.sendMessage(p, "<col=ff000><shad=0202>You last logged in from: " +Server.socketListener.getAddress(p.socket.socket) + ".");
  p.alreadyBanked = false;
				...
				if(!ap.online)
					continue;
				ap.frames.sendMessage(ap, "<img=1><img=1><img=1><col=f000><shad=0202>Server Owner & Coder, <img=1>Chris has logged in!<img=1><img=1><img=1>");
				}
			}
if(p.jailed == 1) {
